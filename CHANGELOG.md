...
- **sandbox.js**: Fixed double shell interpretation vulnerability in `_wrapWithBwrap`. Refactored to use `spawnSync` with args array, eliminating command injection risk from nested shell parsing.

### High-Severity Bug Fixes
- **safety-lead.js / action-safety-lead.js / ai-safety-decide.js**: Replaced fragile JSON extraction regex (`/\{[\s\S]*?\}/` and `/\{[\s\S]*\}/`) with balanced brace-matching algorithm that correctly handles nested JSON objects.
- **ai-safety-decide.js**: Increased tool argument truncation limit from 100 to 500 characters to prevent malicious command tails from being hidden during safety evaluation.
- **agent-teams.js**: Fixed Map-modified-during-iteration bug in `shutdown` — teammate IDs are now collected into an array before iterating. Fixed `Promise.race` orphan promises — all outstanding work is now tracked and awaited via `Promise.allSettled` before `run()` returns.
- **subagent.js**: Wired up `abortSignal` checking in `_execute()` — abort is now checked at the start of each turn and before API calls. Previously the abort mechanism was a complete no-op.
- **ast-search.js**: Fixed shared global regex race condition — `extractSymbols` now creates fresh `RegExp` instances per call instead of reusing module-level regex objects with shared `lastIndex` state.
- **taint-tracker.js**: Removed bidirectional substring match that caused massive false positives (short args like "ls" matched nearly all English text). Added minimum snippet length check (4 chars).
- **rollback.js**: Fixed `messageCount`-based rollback that broke when messages were inserted/removed. Now stores and verifies a SHA-256 fingerprint of the last message at checkpoint time, with nearby-position scanning fallback.
- **web-ui/app.js**: Fixed XSS vulnerability — added `sanitizeHtml()` that strips dangerous tags (`script`, `iframe`, `object`, `embed`, `form`) and event handler attributes before `innerHTML` assignment from `marked.parse()` output.
- **web-server.js**: Fixed CSP blocking CDN scripts — updated `script-src` and `style-src` to allow `https://cdn.jsdelivr.net`. Fixed child process receiving full unfiltered `process.env` — now uses `sanitizeEnv()` to strip API keys and secrets.
- **cli.js**: Fixed web mode passing API key under hardcoded `INCEPTION_API_KEY` regardless of provider — now resolves correct env var name from provider definition (`provider.envKey`).
- **ci.yml**: Fixed `node --test` without file arguments (inconsistent with package.json) — changed to `node --test test/*.test.js`.
- **release.yml**: Fixed heredoc indentation that produced invalid shebang (`#!/usr/bin/env node` had leading spaces) — removed content indentation.

### Medium-Severity Fixes
- **client.js**: Added baseURL fallback chain to `switchProvider` to prevent `undefined` baseURL.
- **conversation.js**: `getUsagePercent` now accepts optional `maxContextTokens` parameter instead of hardcoding Mercury's 128K limit.
- **hooks.js**: Fixed race condition in `load/loadPromise` lifecycle — concurrent callers now share the same promise instead of potentially starting parallel loads.
- **repl.js**: Fixed initial system prompt built without `providerInfo` — now constructs provider info matching `_refreshConversationSystemPrompt`.
- **command-router.js**: `/model` command now calls `normalizeClientConfig` after model assignment to update limits.
- **mcp.js**: Protected security-critical env vars (PATH, HOME, USER, SHELL, TERM) from being overwritten by MCP config.
- **lsp.js**: Moved shared module-level `_requestId` into LspClient as instance property. LSP shutdown now waits for server response before sending exit notification.
- **providers.js**: `registerProvider` now filters `undefined` values before spreading to prevent overwriting computed defaults.
- **subagent.js**: Added `try/catch` for sequential write tool execution (matching `Promise.allSettled` used for parallel reads). Added `decremented` flag to prevent double-decrement of `runningCount`.
- **ast-search.js**: Added symlink loop protection with visited-paths Set in recursive directory traversal.
- **agent-teams.js**: Initialized `error: null` in `addTask()` for documented field. Fixed circular dependency detection by temporarily inserting task before cycle check.
- **taint-tracker.js**: Added LRU-style eviction for `_toolCallTaints` Map (max 1000 entries) to prevent unbounded growth. Increased message scan limit from 8 to 20.
- **web-server.js**: Fixed PowerShell URL injection by escaping single quotes before interpolation.
- **web-child.js**: Added null guard for `repl.sandbox` before calling `init()`.
- **cli.js**: Plan mode now sets `trustMode` to `"readonly"` (valid mode) instead of `"plan"`.
- **memory.js**: Fixed size check using untrimmed vs trimmed content length inconsistency.
- **ci.yml / Makefile**: Fixed `**` glob recursion (replaced with `find` for POSIX compatibility).
- **build-binaries.js**: Pinned `@yao-pkg/pkg` to major version 5 for reproducible builds.

### Low-Severity Fixes & Cleanup
- Removed unused imports: `getDefaultProvider` (config.js), `stat` (project-config.js, ast-search.js), `MODEL_LIMITS` (subagent.js), `fs` (agent-teams.js), `execSync` (executor.js), `readFile`/`writeFile`/`mkdir`/`readdir`/`rm` (rollback.js).
- Unified ANSI color constants in `ui/agent-tabs.js`, `ui/autocomplete.js`, `ui/display.js` — now import from `utils/colors.js` instead of redefining locally.
- Fixed Promise.race timeout timer leaks in `safety-lead.js`, `action-safety-lead.js`, `ai-safety-decide.js` — added `clearTimeout` in `finally` blocks.
- Fixed double error counting in `safety-lead.js` `_failSafeDecision`.
- Added `.codex-backup-*`, `test-output/`, `*.tmp-*` to `.gitignore`.
- Updated Makefile help text to remove hardcoded test count.
- Added misleading comment fix in `windows-installer.iss` regarding WM_SETTINGCHANGE.

### Test Suite Improvements
- **832 tests** across 211 test suites (+12 tests from v1.4.0's 820), all passing:
  - Fixed `async/await` for `_checkPermission` calls in `hardening-regressions.test.js` and `repl-ux.test.js`.
  - Fixed shared `LAB_FEATURES` array mutation leak in `labs.test.js`.
  - Fixed global team registry pollution in `e2e-agents.test.js` with pre-cleanup.
  - Replaced hardcoded version assertion with semver check in `mcp-skills-plan.test.js`.
  - Moved temp directories from project root to `os.tmpdir()` in `security.test.js`.
  - Increased timing-dependent delay from 20ms to 50ms in `history.test.js`.
  - Fixed contradictory test name in `context.test.js` ("preserves" → "summarizes").
  - Added missing assertion in `taint-tracker.test.js`.
  - Changed fragile hardcoded readonly tool allowlist to negative check in `agent-definitions.test.js`.
  - Fixed invalid message sequence in `conversation.test.js`.

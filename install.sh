...
install_mercury_code() {
    ...
    if npm install -g "${script_dir}"; then
        ...
        echo ""
        echo "ERROR: npm install -g failed. This is likely a permissions issue."
        echo ""
        echo "Recommended fixes (choose one):"
        echo "  1. Use nvm (recommended): https://github.com/nvm-sh/nvm"
        echo "  2. Fix npm prefix: npm config set prefix '~/.npm-global'"
        echo "     Then add ~/.npm-global/bin to your PATH"
        echo "  3. If you understand the risks: sudo npm install -g \"${script_dir}\""
        echo ""
        exit 1
    fi
}
...
verify_installation() {
    ...
    # 检查实际导出的命令名
    for alias_cmd in mercury mercury-code; do
...
print_getting_started() {
    ...
    echo ""
    echo -e "     ${GREEN}mercury${RESET}    或    ${GREEN}mercury-code${RESET}"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}文档: https://github.com/LHAMNS/Cloud-code-for-mercury-2${RESET}"

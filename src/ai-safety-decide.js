...
export class AiSafetyDecider {
  ...
  async evaluate(toolName, toolArgs, userTask, conversationHistory = []) {
    ...
    // Each invocation of these tools must be independently evaluated.
    const HIGH_RISK_TOOLS = new Set(["Bash", "Fetch", "SubAgent", "SubAgentTeam"]);

    // Check cache first (with reduced 60-second TTL for security)
    ...
    if (this._decisionCache.has(cacheKey)) {
      ...
      if (Date.now() - cached._cachedAt < 60000) { // 60 second TTL
        // Never serve cached ALLOW decisions for high-risk tools
        if (!(HIGH_RISK_TOOLS.has(toolName) && cached.decision === "ALLOW")) {
          const { _cachedAt, ...result } = cached;
          return result;
        }
      }
      this._decisionCache.delete(cacheKey); // Expired or high-risk bypass
    }
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error('Safety evaluation timed out')), this.timeout);
      });
      let response;
      try {
  ...
  _buildEvalContext(toolName, toolArgs, userTask, conversationHistory) {
    ...
    const argsDisplay = Object.entries(toolArgs)
      .map(([k, v]) => {
        let argsStr = typeof v === "string" ? v : JSON.stringify(v);
        // Strip shell comments before including in eval context
        const cleanArgs = argsStr.replace(/#.*$/gm, '').trim();
        const truncatedArgs = cleanArgs.length > 500 ? cleanArgs.slice(0, 500) + "...[truncated]" : cleanArgs;
        // Block common persuasion patterns
        const safeArgs = truncatedArgs.replace(/\b(?:safe|allow|harmless|standard|routine|normal)\s+(?:operation|command|action)\b/gi, '[redacted-persuasion]');
        return `  ${sanitize(k)}: ${sanitize(safeArgs)}`;
      })
      .join("\n");
...
Evaluate whether this tool call is safe, reasonable, and compliant. Respond with JSON only.
   ...
   * Uses SHA-256 for collision resistance instead of FNV-1a.
   */
  _getCacheKey(toolName, toolArgs, userTask = '') {
    let argsKey;
    try {
      argsKey = typeof toolArgs === "string" ? toolArgs : JSON.stringify(toolArgs ?? {});
    } catch {
      argsKey = String(toolArgs);
    }

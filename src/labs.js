...
class Labs {
   ...
   * @returns {boolean}
   */
  isActive(featureId, visited = new Set()) {
    if (!this.enabled) return false;
    const feature = this.getFeature(featureId);
    if (!feature) return false;

    // Cycle detection — prevent infinite recursion on circular dependencies
    ...
    if (feature.requires) {
      ...
      nextVisited.add(featureId);
      for (const depId of feature.requires) {
        if (!this.isActive(depId, nextVisited)) return false;
      }
    }

...
describe("resolveAgentTools", () => {
  ...
  it("enforces readonly mode", () => {
    ...
    const names = tools.map((t) => t.function.name);
    const writableTools = ["Write", "Edit", "Bash", "Patch"];
    const hasWriteTools = names.some(t => writableTools.includes(t));
    assert.ok(!hasWriteTools, "readonly mode should not include write tools");
  });
});

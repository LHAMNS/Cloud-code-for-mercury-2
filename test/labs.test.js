...
describe("Labs", () => {
  ...
  describe("master switch", () => {
    ...
    it("enableLabs() turns on the master switch", async () => {
      await labs.enableLabs();
      assert.equal(labs.enabled, true);
    });

    it("disableLabs() turns off the master switch", async () => {
      await labs.enableLabs();
      await labs.disableLabs();
      assert.equal(labs.enabled, false);
    });
  });
  ...
  describe("isActive", () => {
    ...
    it("returns true for feature with default=true when labs enabled", async () => {
      await labs.enableLabs();
      assert.equal(labs.isActive("subagent"), true);
    });

    it("returns false for feature with default=false when labs enabled", async () => {
      await labs.enableLabs();
    ...
    it("respects override", async () => {
      await labs.enableLabs();
      await labs.toggle("agent-resume", true);
    ...
    it("fails closed on cyclic dependencies", async () => {
      ...
      try {
        assert.equal(labs.isActive("cycle-a"), false);
      } finally {
        LAB_FEATURES.length = originalLength;
      }
    });
  });
  ...
  describe("isToolAllowed", () => {
    ...
    it("allows gated tools when labs enabled and feature active", async () => {
      await labs.enableLabs();
      assert.equal(labs.isToolAllowed("SubAgent"), true);
      assert.equal(labs.isToolAllowed("SubAgentTeam"), true);
    });

    it("blocks AgentTeams (default=false) even when labs enabled", async () => {
      await labs.enableLabs();
      assert.equal(labs.isToolAllowed("AgentTeams"), false);
    });

    it("allows AgentTeams when explicitly enabled", async () => {
      await labs.enableLabs();
      await labs.toggle("agent-teams", true);
  ...
  describe("toggle", () => {
    it("toggles a feature on", async () => {
      await labs.enableLabs();
      const result = await labs.toggle("agent-resume", true);
    ...
    it("toggles a feature off", async () => {
      await labs.enableLabs();
      const result = await labs.toggle("subagent", false);
      assert.equal(result.ok, true);
      assert.equal(labs.isActive("subagent"), false);
    });

    it("returns error for unknown feature", async () => {
      const result = await labs.toggle("fake-feature");
      assert.equal(result.ok, false);
    });

    it("cascades disable to dependents", async () => {
      await labs.enableLabs();
      await labs.toggle("agent-teams", true);
  ...
  describe("getBlockedTools", () => {
    ...
    it("returns fewer blocked tools when labs enabled", async () => {
      await labs.enableLabs();
      const blocked = labs.getBlockedTools();
  ...
  describe("snapshot", () => {
    ...
    it("shows active features when labs enabled", async () => {
      await labs.enableLabs();
      const snap = labs.snapshot();
      const subagent = snap.find((f) => f.id === "subagent");

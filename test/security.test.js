...
import { resolveViaAncestor } from "../src/utils/path-safety.js";
...
function getHomeDir() {
  return process.env.HOME || process.env.USERPROFILE || os.homedir() || (process.platform === "win32" ? "C:\\Users\\Default" : "/root");
}
...
describe("Security: workspace boundary", () => {
  ...
  it("blocks ListDir outside workspace even without sandbox", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "open" });
    const result = await exec.execute("ListDir", { path: "/etc", max_depth: 1 });
    assert.ok(result.includes("outside workspace") || result.includes("Error"),
      `Should block outside ListDir, got: ${result}`);
  });

  it("blocks Glob outside workspace even without sandbox", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "open" });
    const result = await exec.execute("Glob", { pattern: "*", path: "/etc" });
    assert.ok(result.includes("outside workspace") || result.includes("Error"),
      `Should block outside Glob, got: ${result}`);
  });

  it("blocks Grep outside workspace even without sandbox", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "open" });
    const result = await exec.execute("Grep", { pattern: "root", path: "/etc/passwd" });
    assert.ok(result.includes("outside workspace") || result.includes("Error"),
      `Should block outside Grep, got: ${result}`);
  });

  it("blocks Diff outside workspace even without sandbox", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "open" });
    ...
    assert.ok(result.includes("outside workspace") || result.includes("Error"),
      `Should block outside Diff, got: ${result}`);
  });
});

describe("Security: path traversal defense", () => {
  it("resolveViaAncestor walks up correctly", () => {
    const testPath = path.join(TMPDIR, "nonexistent", "deep", "path.txt");
    const resolved = resolveViaAncestor(testPath);
    // On Windows, /tmp resolves to C:\tmp; use TMPDIR for cross-platform check
    ...
    const resolvedNorm = process.platform === "win32"
      ...
      : resolved;
    assert.ok(resolvedNorm.startsWith(expectedPrefix),
      `Should resolve within ${TMPDIR}, got: ${resolved}`);
  });
...
describe("Security: suspicious command detection", () => {
  ...
  it("blocks shell indirection and eval-style execution", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp", trustMode: "open" });
    const result = await exec.execute("Bash", {
      command: 'PAYLOAD="echo test"; eval "$PAYLOAD"',
    });
    assert.match(result, /indirection|re-evaluation/i);
  });
});

describe("Security: git ref validation", () => {
  it("rejects option-like refs in Diff", async () => {
    const exec = new ToolExecutor({ workspace: "/tmp", trustMode: "open" });
    const result = await exec.execute("Diff", { git_ref: "--ext-diff" });
    assert.match(result, /invalid git ref/i);
  });
});
...
describe("Security: Sandbox path checking", () => {
  ...
  it("allows writes inside a workspace nested under a protected system prefix", () => {
    if (process.platform === "win32") return;

    const sandbox = new Sandbox({ mode: "on", workspace: "/usr/local/project" });
    sandbox.init();

    const check = sandbox.checkPath("/usr/local/project/file.txt", "write");
    assert.equal(check.allowed, true);
  });

  it("on mode blocks reads outside workspace", () => {
    const sandbox = new Sandbox({ mode: "on", workspace: "/tmp/project" });
    sandbox.init();

    const check = sandbox.checkPath("/etc/passwd", "read");
    assert.equal(check.allowed, false);
    assert.ok(check.reason.includes("workspace") || check.reason.includes("temp"));
  });
...
describe("Security: Bash requires real sandbox isolation", () => {
  it("blocks bash when sandbox is enabled without bwrap/firejail", async () => {
    const sandbox = new Sandbox({ mode: "on", workspace: "/tmp" });
    sandbox._capabilities = { bwrap: false, firejail: false, unshare: false };
    const exec = new ToolExecutor({ workspace: "/tmp", trustMode: "open", sandbox });
    const result = await exec.execute("Bash", { command: "echo hello" });
    assert.match(result, /no supported isolation backend/i);
  });

  it("refuses to build bubblewrap shell strings", () => {
    const sandbox = new Sandbox({ mode: "on", workspace: "/tmp" });
    sandbox._capabilities = { bwrap: true, firejail: false, unshare: false };
    assert.throws(() => sandbox.wrapCommand("echo hello"), /spawnCommand/);
  });

  it("uses sandbox.spawnCommand for isolated bash execution", async () => {
    let called = false;
    const sandbox = {
      enabled: true,
      hasCommandIsolation: () => true,
...
describe("Security: sandbox runtime configuration", () => {
  it("mode and workspace stay mutable after init for REPL/worktree updates", () => {
    const sandbox = new Sandbox({ mode: SANDBOX_ON, workspace: "/tmp/original" });
    sandbox.init();

    sandbox.mode = SANDBOX_STRICT;
    sandbox.workspace = "/tmp/updated";

    assert.equal(sandbox.mode, SANDBOX_STRICT);
    assert.equal(sandbox.workspace, "/tmp/updated");
  });
});
...
describe("Security: one-time outside-workspace bypass (token-based)", () => {
  it("_outsideWriteTokens defaults to empty Map", () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "approval" });
    assert.ok(exec._outsideWriteTokens instanceof Map);
    assert.equal(exec._outsideWriteTokens.size, 0);
  });

  it("grantOutsideWriteToken creates consumable token", () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "approval" });
    const token = exec.grantOutsideWriteToken();
    assert.equal(typeof token, "string");
    assert.equal(token.length, 32); // 16 bytes hex = 32 chars
    assert.ok(exec._outsideWriteTokens.has(token));
  });

  it("token is consumed after use and blocks further outside writes", () => {
    const exec = new ToolExecutor({ workspace: "/tmp/test-ws", trustMode: "approval" });
    const token = exec.grantOutsideWriteToken();
    assert.equal(exec._outsideWriteTokens.size, 1);
    // Revoke it
    exec.revokeOutsideWriteToken(token);
    assert.equal(exec._outsideWriteTokens.size, 0);
  });
});
...
describe("Hardened checkPath", () => {
  ...
  it("strict mode allows reads in tmp dir", () => {
    const sandbox = new Sandbox({ mode: SANDBOX_STRICT, workspace: path.join(TMPDIR, "test-ws") });
    sandbox.init();
    const result = sandbox.checkPath(path.join(TMPDIR, "other-file.txt"), "read");
...
describe("Sandbox symlink policy", () => {
  ...
  it("allow policy skips symlink check", () => {
    ...
    const result = sandbox.checkSymlink("/tmp/test-ws/any-path");
    assert.equal(result.allowed, true);
  });
...
describe("Sandbox security event logging", () => {
  it("logs security events on blocked operations", () => {
    ...
    // Use a large count to include all events (Windows generates many
    // ancestor_resolution_failed events during checkPath which can push
    // earlier events out of the default 50-event window).
    const events = sandbox.getSecurityEvents(200);
    assert.ok(events.length >= 2, `Should have logged events, got ${events.length}`);
    assert.ok(events.some(e => e.type === "http_blocked"),
      `Should log http_blocked, got types: ${[...new Set(events.map(e => e.type))]}`);
    assert.ok(events.some(e => e.type === "strict_boundary_blocked"),
      `Should log strict_boundary_blocked, got types: ${[...new Set(events.map(e => e.type))]}`);
  });
...
describe("Security: grep include handling", () => {
  it("matches nested include patterns relative to the original search root", async () => {
    const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-test-grep-"));
    const nestedDir = path.join(workspace, "src", "nested");
    const targetFile = path.join(nestedDir, "file.js");
    fs.mkdirSync(nestedDir, { recursive: true });
    fs.writeFileSync(targetFile, "needle\n", "utf-8");

    try {
      const exec = new ToolExecutor({ workspace, trustMode: "open" });
      const result = await exec.execute("Grep", {
        pattern: "needle",
        path: workspace,
        include: "src/**/*.js",
      });
      assert.ok(result.includes(targetFile), `Expected nested file match, got: ${result}`);
    } finally {
...
describe("Security: diff fallback", () => {
  it("diffs two files even when the system diff binary is unavailable", async () => {
    const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-test-diff-"));
    const fileA = path.join(workspace, "a.txt");
    const fileB = path.join(workspace, "b.txt");
    fs.writeFileSync(fileA, "one\n", "utf-8");
    fs.writeFileSync(fileB, "two\n", "utf-8");

    try {
      const exec = new ToolExecutor({ workspace, trustMode: "open" });
      const result = await exec.execute("Diff", { file_a: fileA, file_b: fileB });
      assert.ok(result.includes("-one") || result.includes("+two"),
        `Expected unified diff output, got: ${result}`);
    } finally {
      fs.rmSync(workspace, { recursive: true, force: true });
    }
  });
